...
- **VRAM:** 24 ГБ+ рекомендуется; 16 ГБ поддерживается (авто-чанкование, чуть медленнее рендер); 12 ГБ экспериментально (требуется `YUE2_ALLOW_12GB=1`)[cite: 4]. *Примечание: Есть вероятность стабильной работы на 11 ГБ RTX 2080 Ti.*
